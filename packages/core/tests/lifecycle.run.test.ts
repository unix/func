...
test.sequential(
  'should run default handler for a command',
  async ({ runContainer }) => {
    let invoked = false
    const name = random()
    @Command({ name })
...
test.sequential(
  'should run major handler with inferred values and defaults',
  async ({ runContainer }) => {
    expect.assertions(4)
    @CommandMajor()
    class Major {
...
test.sequential(
  'should reject multiple selected handlers',
  async ({ runContainer }) => {
    expect.assertions(2)
...
test.sequential(
  'should support required and validate for values',
  async ({ runContainer }) => {
    expect.assertions(2)
    const log = vi.spyOn(console, 'error').mockImplementation(() => {})
    @CommandMajor()
...
test.sequential(
  'should keep sub-options as manual parse schema',
  async ({ runContainer }) => {
    expect.assertions(1)
    @CommandMajor()
...
test.sequential(
  'should throw duplicate errors across sub-options and values',
  async () => {
    @CommandMajor()
    ...
    expect(() => new Container([Major])).toThrow(
      expect.objectContaining({ code: F_SYSTEM.DUPLICATE_OPTION_TOKEN }),
    )
  },
)
...
test.sequential('should allow only one major command', () => {
  ...
  expect(() => new Container([FirstMajor, SecondMajor])).toThrow(
    expect.objectContaining({ code: F_SYSTEM.MULTIPLE_MAJOR_COMMANDS }),
  )
})

test.sequential('should throw when no handler matches the invocation', async () => {
  @CommandMajor()
  class Major {
    @Handler({ flag: 'help' })
    help() {}
  ...
  await expect(new Container([Major], { argv: [] }).run()).rejects.toMatchObject({
    code: F_SYSTEM.NO_MATCHING_HANDLER,
  })
})
