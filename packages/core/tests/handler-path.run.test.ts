import { Args, Command, FuncArgs, Handler } from '../src'
import { expect, random, test } from './_test'

test.sequential(
  'should select a handler by path before default handler',
  async ({ runContainer }) => {
    expect.assertions(3)
    const name = random()
...
test.sequential(
  'should support multi-segment handler paths',
  async ({ runContainer }) => {
    expect.assertions(1)
    const name = random()
...
test.sequential(
  'should select flag handlers after path handlers',
  async ({ runContainer }) => {
    let selected = ''
    const name = random()
