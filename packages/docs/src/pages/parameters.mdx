...
Do not inject `@Exception()` into an ordinary handler: there is no current failure there. Error flow and default printing are covered in [Error Handling](/error-handler).
